    ...
    try:
        ...
        if mode == "combined_emperor_actions":
            ...
            task = (
                ...
                "每項給：title（一句完整的主謂賓句子，讓讀者不必展開說明即可知道皇帝『對誰做了什麼』：以動詞＋具體受詞／內容書寫，"
                "主詞若為皇帝可省略但必須保留動詞與受詞；約20-40字，須點名相關官員與具體事由，"
                "例如『指示黃仕簡等俟各路官兵到齊後約期同時併力夾攻』『駁斥常青水陸兩提督俱渡重洋、過度調兵之議』『申飭常青等辦理逆案過於張皇失措』；"
                "不得使用『針對…之指示』『對…之部署』『關於…之評估』這類看不出實際內容的空泛標題，也不得在標題末尾加註括號補充或標籤如『（張皇）』）；description（1-3句）；"
        ...
        if mode == "official_response":
            ...
            response_gate = (
                "每份候選都與此上諭已有配對關係；請保留並分析每份文書。從中找出官員自己的答覆／遵行／申辯原文，"
                "不要把其重引的上諭當作 response_quote；若一份文書只明示奉旨而答覆很簡略，仍據實描述為知悉或簡略覆奏，不得虛構細節。"
                "重點：官員回應須同時扣合『皇帝行動引文』中的上諭指示與其所回應的奏摺原報之事——"
                "即該官員文書要確實回覆／續報／辦理『上諭針對該原報所發之指示或所問之事』，而非僅提及上諭中另一無關事項；"
                "若某已配對文書內容其實與此原報之事無關，請於 how 中據實說明其關聯薄弱，不要勉強拼湊回應細節。"
                if confirmed_only else
