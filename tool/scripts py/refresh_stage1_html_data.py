...
def main() -> None:
    ...
    refreshed = replace_if_present(
        refreshed,
        "</span></div>'+((isZhu)?'':(yuRecips.length?'<div class=\"dp-l2\">'+dpFld(yuId,'recipients',yuRecips.join('、'))+'</div>':''))",
        "</span></div>' +(isZhu&&yuSender?'<div class=\"dp-l2\">'+dpFld(yuId,'author_name',yuSender)+'</div>':'') + ((isZhu)?'':(yuRecips.length?'<div class=\"dp-l2\">'+dpFld(yuId,'recipients',yuRecips.join('、'))+'</div>':''))",
        "Zhu sender line",
    )
    refreshed = replace_if_present(
        refreshed,
