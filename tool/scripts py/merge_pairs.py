#!/usr/bin/env python3
"""Merge confirmed 上諭／硃批 response pairs into the source JSON.
...
`responds_to` entry on the replying document in stage1_original_text.json, plus a
reciprocal `responses` entry on the 上諭.

Usage:
  python3 "tool/scripts py/merge_pairs.py"            # merge everything you've confirmed
  python3 "tool/scripts py/merge_pairs.py" --dry-run  # show what would change, write nothing
"""
...
SOURCE_PATH = ATTEMPT / "stage1_original_text.json"
...
def main() -> int:
    ...
    ap.add_argument("--source", type=Path, default=SOURCE_PATH,
                    help="Source JSON to write into (default: stage1_original_text.json).")
    ...
    for p in pairs:
        ...
        relation_type = "硃批" if relation == "official_reply_to_emperor_zhu" else "上諭"
