...
read their prompts from tool/skills md/*.md. This entry point only selects the 硃批
records by date, invokes that proven runner, and annotates its bundle with the
period-specific loop metadata.

Example:
  python3 "tool/scripts py/run_zhu_review_loop.py" \
...
def main() -> int:
    ...
    manifest.update({
        "loop_skill": "tool/skills md/zhu-review-loop.md",
