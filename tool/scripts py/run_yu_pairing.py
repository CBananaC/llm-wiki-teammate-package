...
match_level.  See tool/skills md/yu-response-pairing.md.

Detection runs on the date-rich corpus (dual-timeline-data.json).  Write-back
(a later phase) targets stage1_original_text.json.

Examples:
  # structural preview, no proxy call:
  python3 "tool/scripts py/run_yu_pairing.py" --doc-id 天43 --dry-run
  # full run against the proxy:
  python3 "tool/scripts py/run_yu_pairing.py" --proxy https://... --doc-id 天43
  # whole corpus:
  python3 "tool/scripts py/run_yu_pairing.py" --proxy https://... --all
"""
...
PROMPT_PATH = ROOT / "tool" / "skills md" / "yu-response-pairing.md"
