...
instructions are enforced in `tool/proxy/gemini-proxy/main.py` `mode:"trace"`.
...
- **Quotation display.** The website reconstructs one continuous verbatim passage
  ...
  When the first hop quote begins at a nested 「據供／據稱」 clause, the renderer
  follows the proxy rule **作者引介來源的完整引文** and
  may also include the immediately preceding author-written source-identification
  sentence if it names the informant (for example 「茲於本月十六日據防卡官兵拿獲奸細僧人西葉、心向、新法三名」).
  This lead-in is supporting context, not a required extra hop. Never invent a
  source-identification sentence when the original text does not contain one.
...
- Terminal: `tool/scripts py/run_review_bundle_test.py --steps source-chain`
- Website: 事件卡片中的「來源鏈追溯」
- Proxy: `tool/proxy/gemini-proxy/main.py`, `mode: "trace"`, optional extra focus via
